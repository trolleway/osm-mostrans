...
def filter_osm_dump():
    ...
    ~/osmosis/bin/osmosis \
      ...
      --tf reject-relations type="route_stopped"
      --used-way --used-node \
...
if __name__ == '__main__':
        ...
        os.system('python update_ngw_from_geojson.py  --ngw_url '+config.ngw_url+' --ngw_resource_id 676 --ngw_login '+config.ngw_login+' --ngw_password '+config.ngw_password+' --check_field road_id --filename routes_with_refs.geojson')
        os.system('python update_ngw_from_geojson.py  --ngw_url '+config.ngw_url+' --ngw_resource_id 677 --ngw_login '+config.ngw_login+' --ngw_password '+config.ngw_password+' --check_field terminal_id --filename terminals_export.geojson')


#CREATE INDEX planet_osm_rels_gin_trgm_idx  ON planet_osm_rels USING gin  (members );#разобраться с оператором для gin
#vacuum analyze    
